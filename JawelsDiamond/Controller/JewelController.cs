...
namespace JawelsDiamond.Controller
{
    public class JewelController
    {
        ...
        public string AddJewel(string name, int categoryId, int brandId, string price, string year)
        {
            if (string.IsNullOrEmpty(name) || categoryId == 0 || brandId == 0 ||
                string.IsNullOrEmpty(price) || string.IsNullOrEmpty(year))
            {
            ...
            JewelHandler.AddJewel(brandId, categoryId, name, jewelPrice, releaseYear);
            return "Success";
        }
    }
}